...
const GenerateMcqsFromUploadedMaterialInputSchema = z.object({
  ...
  level: z.string().describe('The level of the CA exam: Foundation, Intermediate, or Final.'),
  subject: z.string().describe('The subject for which MCQs are generated.'),
...
const generateMcqsPrompt = ai.definePrompt({
  ...
  Generate {{count}} MCQs for the {{subject}} subject at the {{level}} level with {{difficulty}} difficulty, based on the content of the uploaded study material. Each MCQ should have four options (A, B, C, D), a correct answer, and a detailed explanation.
